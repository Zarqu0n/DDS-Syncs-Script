...
public:
    ...
    bool init(const std::string& wan_ip,
        unsigned short port,const std::string& topic_name)
    {   
        DomainParticipantQos pqos;
        pqos.wire_protocol().builtin.discovery_config.leaseDuration = eprosima::fastrtps::c_TimeInfinite; //keşif süresini sonsuz yapar
        pqos.wire_protocol().builtin.discovery_config.leaseDuration_announcementperiod =
                eprosima::fastrtps::Duration_t(5, 0); // keşif süresi bildirim süresi
        pqos.name("Participant_pub");

        pqos.transport().use_builtin_transports = false;

        std::shared_ptr<TCPv4TransportDescriptor> descriptor =
                         std::make_shared<TCPv4TransportDescriptor>(); // TCPv4 transpor kullanan bir nesne oluşturuyoruz

        //Nesnenin buffer boyutlarını yaplandırma
        descriptor->sendBufferSize = 0;
        descriptor->receiveBufferSize = 0;

        //Nesnenin IP adresini ve portunu yaplandırma
        descriptor->set_WAN_address(wan_ip);
        std::cout << "[RTPS] Setting Publisher on :" << wan_ip << "@" << port << std::endl;
        ...
        wqos.history().depth = 30;// Saklanacak veri sayısı
        wqos.resource_limits().max_samples = 50; // aynı anda gönderilebilecek veri sayısı
    ...
    void run(std::string key,std::string value)
    {
        ...
        keyvalue_.key(key);
        std::cout << "[RTPS] SENT-> KEY: " << keyvalue_.value() 
                    <<"    VALUE:   " << keyvalue_.key()<< std::endl;
...
public:
    ...
    virtual ~KeyValueSubscriber()
    {
        ...
        */
        ...
        // TCPv4 transport konfigürasyonu
        DomainParticipantQos pqos;
        ...
        std::cout << "[RTPS]Subscriber connected:"<<wan_ip << "@" << port << std::endl;
        ...
        rqos.history().depth = 30;// Saklanacak veri sayısı
        rqos.resource_limits().max_samples = 50;// aynı anda gönderilebilecek veri sayısı
        rqos.resource_limits().allocated_samples = 20;// aynı anda gönderilebilecek veri sayısı
...
void publisher(const std::string& wan_ip,
        unsigned short port,const std::string& topic_name)
{
    Singleton& singleton = Singleton::getInstance();

    KeyValuePublisher* mypub = new KeyValuePublisher();
    mypub->init(wan_ip, port, topic_name);
...
{   
    ...
    if (moduleName == NULL)
    {
        std::cout << "[SYS]Error: Python script name is not valid." << std::endl;
        return;
    }
...
int main()
{
    ...
    unsigned short subscriber_port = 5001;
    unsigned short publisher_port = 5002;
    ...
    std::thread pub_thread(&publisher,ip,publisher_port,Publisher_topic_name);
    std::thread sub_thread(&subscriber,ip,subscriber_port,Subscriber_topic_name);
